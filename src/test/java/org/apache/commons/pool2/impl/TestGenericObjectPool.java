...
public class TestGenericObjectPool extends TestBaseObjectPool {
	...
	public static BasePooledObjectFactory<Object> mockBasePooledObjectFactory3() throws Exception {
		...
		doReturn(new Object()).when(mockInstance).create();
		doAnswer((stubInvo) -> {
	...
	public static BasePooledObjectFactory<HashSet<String>> mockBasePooledObjectFactory2() throws Exception {
		BasePooledObjectFactory<HashSet<String>> mockInstance = spy(BasePooledObjectFactory.class);
		doAnswer((stubInvo) -> {
	...
	static class TestThread<T> implements Runnable {
		...
		public TestThread(final ObjectPool<T> pool, final int iter, final int startDelay, final int holdTime,
		...
		public void run() {
			for (int i = 0; i < _iter; i++) {
				final long startDelay = _randomDelay ? (long) _random.nextInt(_startDelay) : _startDelay;
				final long holdTime = _randomDelay ? (long) _random.nextInt(_holdTime) : _holdTime;
				try {
	...
	private static final boolean DISPLAY_THREAD_DETAILS = Boolean
			.valueOf(System.getProperty("TestGenericObjectPool.display.thread.details", "false")).booleanValue();
	...
	private void assertConfiguration(final GenericObjectPoolConfig<?> expected, final GenericObjectPool<?> actual)
		...
		assertEquals("whenExhaustedAction", Boolean.valueOf(expected.getBlockWhenExhausted()),
		...
		assertEquals("minEvictableIdleTimeMillis", expected.getMinEvictableIdleTimeMillis(),
				actual.getMinEvictableIdleTimeMillis());
		assertEquals("numTestsPerEvictionRun", expected.getNumTestsPerEvictionRun(),
				actual.getNumTestsPerEvictionRun());
		assertEquals("evictorShutdownTimeoutMillis", expected.getEvictorShutdownTimeoutMillis(),
				actual.getEvictorShutdownTimeoutMillis());
		assertEquals("timeBetweenEvictionRunsMillis", expected.getTimeBetweenEvictionRunsMillis(),
	...
	@Override
	protected ObjectPool<Object> makeEmptyPool(final PooledObjectFactory<Object> fac) {
		return new GenericObjectPool<>(fac);
	}

	/**
	 * Kicks off <numThreads> test threads, each of which will go through
	 * <iterations> borrow-return cycles with random delay times <= delay in
	 * between.
	 */
	@SuppressWarnings({ "rawtypes", "unchecked" })
	private void runTestThreads(final int numThreads, final int iterations, final int delay,
			final GenericObjectPool testPool) {
	...
	public void testClientWaitStats() throws Exception {
		...
		try (final GenericObjectPool<String> pool = new GenericObjectPool<>(factory,
				new GenericObjectPoolConfig<String>())) {
	...
	public void testConcurrentBorrowAndEvict() throws Exception {
		...
		for (int i = 0; i < 5000; i++) {
			...
			/*
			 * Uncomment this for a progress indication if (i % 10 == 0) {
	...
	public void testEqualsIndiscernible() throws Exception, Exception {
		final BasePooledObjectFactory<HashSet<String>> factory = TestGenericObjectPool.mockBasePooledObjectFactory2();
	...
	public void testEviction() throws Exception {
		...
		assertTrue("Should be less than 500 idle, found " + genericObjectPool.getNumIdle(),
				genericObjectPool.getNumIdle() < 500);
		try {
			Thread.sleep(600L);
		} catch (final InterruptedException e) {
		}
		assertTrue("Should be less than 400 idle, found " + genericObjectPool.getNumIdle(),
				genericObjectPool.getNumIdle() < 400);
		try {
			Thread.sleep(600L);
		} catch (final InterruptedException e) {
		}
		assertTrue("Should be less than 300 idle, found " + genericObjectPool.getNumIdle(),
				genericObjectPool.getNumIdle() < 300);
		try {
			Thread.sleep(600L);
		} catch (final InterruptedException e) {
		}
		assertTrue("Should be less than 200 idle, found " + genericObjectPool.getNumIdle(),
				genericObjectPool.getNumIdle() < 200);
		try {
			Thread.sleep(600L);
		} catch (final InterruptedException e) {
		}
		assertTrue("Should be less than 100 idle, found " + genericObjectPool.getNumIdle(),
				genericObjectPool.getNumIdle() < 100);
		try {
			Thread.sleep(600L);
		} catch (final InterruptedException e) {
		}
	...
	@Test(timeout = 60000)
	public void testEvictionInvalid() throws Exception, Exception {

		try (final GenericObjectPool<Object> invalidFactoryPool = new GenericObjectPool<>(
				TestGenericObjectPool.mockBasePooledObjectFactory3())) {
	...
	public void testEvictionWithNegativeNumTests() throws Exception {
		...
		assertTrue("Should at most 6 idle, found " + genericObjectPool.getNumIdle(),
				genericObjectPool.getNumIdle() <= 6);
		try {
			Thread.sleep(100L);
		} catch (final InterruptedException e) {
		}
		assertTrue("Should at most 3 idle, found " + genericObjectPool.getNumIdle(),
				genericObjectPool.getNumIdle() <= 3);
		try {
			Thread.sleep(100L);
		} catch (final InterruptedException e) {
		}
		assertTrue("Should be at most 2 idle, found " + genericObjectPool.getNumIdle(),
				genericObjectPool.getNumIdle() <= 2);
		try {
			Thread.sleep(100L);
		} catch (final InterruptedException e) {
		}
	...
	public void testJmxRegistration() {
		...
		try (final GenericObjectPool<String> poolWithDefaultJmxNameBase = new GenericObjectPool<>(simpleFactory,
				config)) {
	...
	/**
	 * Test the following scenario: Thread 1 borrows an instance Thread 2 starts to
	 * borrow another instance before thread 1 returns its instance Thread 1 returns
	 * its instance while thread 2 is validating its newly created instance The test
	 * verifies that the instance created by Thread 2 is not leaked.
	 *
	 * @throws Exception May occur in some failure modes
	 */
	@Test(timeout = 60000)
	...
	/*
	 * Test multi-threaded pool access. Multiple threads, but maxTotal only allows
	 * half the threads to succeed.
	 *
	 * This test was prompted by Continuum build failures in the Commons DBCP test
	 * case: TestPerUserPoolDataSource.testMultipleThreads2() Let's see if the this
	...
	public void testMaxWaitMultiThreaded() throws Exception {
		...
		if (DISPLAY_THREAD_DETAILS || wtt.length / 2 != failed) {
			System.out.println("MaxWait: " + maxWait + " HoldTime: " + holdTime + " MaxTotal: " + threads + " Threads: "
	...
	public void testMultipleReturnOfSameObject() throws Exception {
		try (final GenericObjectPool<String> pool = new GenericObjectPool<>(simpleFactory,
				new GenericObjectPoolConfig<String>())) {
	...
	/**
	 * This is the test case for POOL-263. It is disabled since it will always pass
	 * without artificial delay being injected into GOP.returnObject() and a way to
	 * this hasn't currently been found that doesn't involve polluting the GOP
	 * implementation. The artificial delay needs to be inserted just before the
	 * final call to isLifo() in the returnObject() method.
	 */
	// @Test(timeout = 60000)
